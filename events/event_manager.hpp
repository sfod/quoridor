...
class EventManager {
    typedef bs2::signal<void (const std::shared_ptr<EventDataBase>&)> signal_t;
...
public:
    ...
    bool trigger_event(const std::shared_ptr<EventDataBase> &event) const;
...
private:
    ...
    std::list<std::shared_ptr<EventDataBase>> event_list_;
};
